...
\section{Lecture 1 --- Probability refresher: information, conditioning, and basic tools}
...
\item \textit{Stochastic Calculus for Finance II: Continuous-Time Models}, S. E. Shreve, Chapters~1 \& 2, pp.~ 1-77
...
\paragraph{Events and information.}
We work on a probability space $(\Omega,\mathcal F,\Pbb)$.
An \emph{event} is a set $A\subseteq\Omega$ and represents a statement about the outcome.
A \emph{$\sigma$-algebra} $\mathcal G\subseteq\mathcal F$ models the information available to an agent:
\[
\Omega\in\mathcal G,\qquad A\in\mathcal G \Rightarrow A^c\in\mathcal G,\qquad (A_n)\subset\mathcal G \Rightarrow \bigcup_n A_n\in\mathcal G.
\]
More events in $\mathcal G$ means \emph{more information}; if $A\notin\mathcal G$, then $A$ cannot be decided with information $\mathcal G$.

\paragraph{Generated $\sigma$-algebra.}
Given a collection of events $\mathcal C\subseteq\mathcal F$, the $\sigma$-algebra it generates is the smallest $\sigma$-algebra containing $\mathcal C$:
\[
\sigma(\mathcal C)
:=\bigcap\{\mathcal H\subseteq\mathcal F:\ \mathcal H\text{ $\sigma$-algebra and }\mathcal C\subseteq\mathcal H\}.
\]
A key special case is a single event $A$:
\[
\sigma(\{A\})=\{\emptyset,A,A^c,\Omega\},
\]
which corresponds to the yes/no information “did $A$ happen?”.

\paragraph{Measurability = observability.}
A random variable $Y:(\Omega,\mathcal F)\to(\R,\mathcal B(\R))$ is \emph{$\mathcal G$-measurable} if
\[
\{Y\le t\}\in\mathcal G \quad \forall t\in\R,
\]
equivalently $\{Y\in B\}\in\mathcal G$ for all Borel sets $B$.
Interpretation: $Y$ can be computed using only the information contained in $\mathcal G$.

\paragraph{Finite spaces: partitions and atoms.}
On a finite $\Omega$, a $\sigma$-algebra corresponds to a partition into \emph{atoms}
(indistinguishable blocks). If $\Pi=(A_i)$ is a partition, then
\[
\sigma(\Pi)=\Big\{\bigcup_{i\in J}A_i:\ J\subseteq I\Big\},
\]
and any $\mathcal G=\sigma(\Pi)$-measurable variable has the form
\[
Y=\sum_i c_i\,\mathbf 1_{A_i},
\]
i.e.\ it is constant on each atom.

\subsection{Filtrations and adapted processes}

\paragraph{Information over time.}
A \emph{filtration} $(\mathcal F_t)_{t\ge 0}$ is an increasing family of $\sigma$-algebras:
\[
\mathcal F_s\subseteq\mathcal F_t\quad (s\le t),
\]
where $\mathcal F_t$ represents the information available up to time $t$.

\paragraph{Adaptedness (no look-ahead).}
A process $(X_t)_{t\ge 0}$ is \emph{adapted} to $(\mathcal F_t)$ if
\[
X_t \text{ is }\mathcal F_t\text{-measurable for each }t.
\]
Finance intuition: a trading decision at time $t$ must depend only on information available at time $t$.

\paragraph{Canonical filtration.}
Given a process $X$, its natural (canonical) filtration is
\[
\mathcal F^X_t := \sigma(X_s:\ s\le t),
\]
the smallest $\sigma$-algebra that makes the past of $X$ observable.

\subsection{Conditioning: conditional probability, conditional distributions, conditional expectation}

\paragraph{Conditional probability (event conditioning).}
For events $A,B$ with $\Pbb(B)>0$,
\[
\Pbb(A\mid B)=\frac{\Pbb(A\cap B)}{\Pbb(B)}.
\]
Interpretation: restrict to the scenario $B$ and renormalize.

\paragraph{Conditional distributions.}
\begin{itemize}
\item \textbf{Discrete case:} for discrete $(X,Y)$ and $\Pbb(Y=y)>0$,
\[
\Pbb(X=x\mid Y=y)=\frac{\Pbb(X=x,Y=y)}{\Pbb(Y=y)}.
\]
\item \textbf{Density case:} if $(X,Y)$ has joint density $f_{X,Y}$ and $f_Y(y)>0$,
\[
f_{X\mid Y=y}(x)=\frac{f_{X,Y}(x,y)}{f_Y(y)}.
\]
\end{itemize}
If $X$ and $Y$ are independent, conditioning does not change the law: $f_{X\mid Y=y}=f_X$.

\paragraph{Conditional expectation given a $\sigma$-algebra.}
For $X\in L^1(\Omega,\mathcal F,\Pbb)$ and $\mathcal G\subseteq\mathcal F$,
the conditional expectation $\E[X\mid\mathcal G]$ is the (a.s.\ unique) $\mathcal G$-measurable random variable $Z$ such that
\[
\int_A Z\,d\Pbb=\int_A X\,d\Pbb \qquad \forall A\in\mathcal G.
\]
Interpretation: $\E[X\mid\mathcal G]$ is the best prediction of $X$ using only information $\mathcal G$.

\paragraph{Conditioning on a random variable.}
If $\mathcal G=\sigma(Y)$, then $\E[X\mid\sigma(Y)]$ is a measurable function of $Y$:
\[
\E[X\mid\sigma(Y)] = m(Y).
\]
A useful characterization (test-function form) is:
\[
\E[m(Y)\,\varphi(Y)] = \E[X\,\varphi(Y)]
\quad \text{for all bounded measurable }\varphi.
\]

\paragraph{Three identities used constantly.}
For integrable $X,X'$, $\mathcal H\subseteq\mathcal G\subseteq\mathcal F$, and $Z$ $\mathcal G$-measurable:
\[
\E[aX+X'\mid\mathcal G]=a\,\E[X\mid\mathcal G]+\E[X'\mid\mathcal G],
\]
\[
\E[ZX\mid\mathcal G]=Z\,\E[X\mid\mathcal G]\qquad\text{(taking out what is known)},
\]
\[
\E\!\left[\E[X\mid\mathcal G]\mid\mathcal H\right]=\E[X\mid\mathcal H]\qquad\text{(tower property)}.
\]
These are the algebra behind dynamic pricing and martingales.

\subsection{Stochastic processes: discrete vs continuous time (definitions)}

\paragraph{Discrete-time processes.}
A discrete-time stochastic process is a sequence $(X_n)_{n\ge 0}$.
A natural filtration is $\mathcal F_n:=\sigma(X_0,\dots,X_n)$.
Example: the simple random walk $S_n=\sum_{k=1}^n \varepsilon_k$ with i.i.d.\ $\varepsilon_k\in\{-1,+1\}$.

\paragraph{Continuous-time processes.}
A continuous-time process is $(X_t)_{t\ge 0}$.
One often assumes path regularity (continuous paths or c\`adl\`ag) and adaptedness.
Brownian motion is announced as the central continuous-time building block (defined later).

\subsection{Stopping times and $\mathcal F_\tau$}

\paragraph{Stopping times (no look-ahead random times).}
\begin{itemize}
\item \textbf{Discrete time:} $\tau:\Omega\to\{0,1,2,\dots\}\cup\{\infty\}$ is a stopping time if
\[
\{\tau\le n\}\in\mathcal F_n\quad \forall n\ge 0.
\]
\item \textbf{Continuous time:} $\tau:\Omega\to[0,\infty]$ is a stopping time if
\[
\{\tau\le t\}\in\mathcal F_t\quad \forall t\ge 0.
\]
\end{itemize}
Typical examples: first hitting times $\inf\{t:\ S_t\ge a\}$, exit times from an interval, default times, (later) exercise times for American options.
Non-examples are definitions requiring future values (look-ahead).

\paragraph{Information at a random time.}
For a stopping time $\tau$, define the $\sigma$-algebra of information available when stopping:
\[
\mathcal F_\tau
:=\{A\in\mathcal F:\ A\cap\{\tau\le t\}\in\mathcal F_t\ \text{for all }t\ge 0\},
\]
(and similarly in discrete time).
Sanity checks: if $\tau\equiv t_0$ then $\mathcal F_\tau=\mathcal F_{t_0}$; if $\tau\le t$ then $\mathcal F_\tau\subseteq\mathcal F_t$.

\subsection{Martingales (definition + intuition + optional stopping preview)}

\paragraph{Martingale / submartingale / supermartingale.}
An integrable adapted process $(X_t)$ is a martingale (resp.\ sub-/supermartingale) if for all $s\le t$,
\[
\E[X_t\mid\mathcal F_s]=X_s
\quad(\text{resp. }\ge X_s,\ \le X_s).
\]
Interpretation: martingale = fair game (no predictable drift).

\paragraph{Stopped process and optional stopping (safe version).}
Given a martingale $(M_t)$ and a stopping time $\tau$, the stopped process
\[
M_t^\tau:=M_{t\wedge \tau}
\]
is again a martingale.
A preview of the Optional Stopping Theorem is given in the bounded case:
if $\tau$ is bounded, then $\E[M_\tau]=\E[M_0]$.

\subsection{Statistical tools for pricing: LLN and CLT (Monte Carlo viewpoint)}

\paragraph{Law of Large Numbers (LLN).}
For i.i.d.\ $(X_i)$ with $\E[|X_1|]<\infty$,
\[
\widehat\mu_n:=\frac1n\sum_{i=1}^n X_i \to \mu:=\E[X_1].
\]
This justifies estimating expectations by empirical means, including Monte Carlo pricing
\[
V_0=\E[g(X)]\approx \widehat V_n:=\frac1n\sum_{i=1}^n g(X^{(i)}).
\]

\paragraph{Central Limit Theorem (CLT) and confidence intervals.}
If $\Var(X_1)=\sigma^2\in(0,\infty)$, then
\[
\sqrt{n}\,(\widehat\mu_n-\mu)\Rightarrow \mathcal N(0,\sigma^2),
\qquad
\widehat\mu_n\approx \mathcal N\!\Big(\mu,\frac{\sigma^2}{n}\Big).
\]
This yields the rule-of-thumb Monte Carlo error $\sim 1/\sqrt{n}$ and approximate confidence intervals:
\[
\widehat\mu_n \pm 1.96\,\frac{\widehat\sigma}{\sqrt{n}},
\qquad
\widehat\sigma^2=\frac{1}{n-1}\sum_{i=1}^n (X_i-\widehat\mu_n)^2.
\]
Practical warning: dependence, heavy tails, and non-stationarity can invalidate naive CLT diagnostics.

\subsection{CDF and empirical CDF}
...
If a density $f_X$ exists, then $F_X(x)=\int_{-\infty}^x f_X(u)\,du$ and $f_X=F_X'$ a.e.

\paragraph{Empirical CDF.}
From data $X_1,\dots,X_n$,
\[
\widehat F_n(x):=\frac1n\sum_{i=1}^n \mathbf 1_{\{X_i\le x\}},
\]
which converges pointwise to $F_X$ by LLN.
Quantiles (e.g.\ VaR-type objects) can be defined by
\[
q_\alpha:=\inf\{x:\ F_X(x)\ge \alpha\}.
\]

\subsection{Gaussian vectors (multivariate normal)}

\paragraph{Definition and characterization.}
A vector $X\in\R^d$ is Gaussian if every linear combination $a^\top X$ is Gaussian.
Its law is fully determined by its mean $m\in\R^d$ and covariance matrix $\Sigma\in\R^{d\times d}$:
\[
X\sim\mathcal N(m,\Sigma).
\]
Stability under affine maps: for $A\in\R^{k\times d}$ and $b\in\R^k$,
\[
AX+b\sim \mathcal N(Am+b,\ A\Sigma A^\top).
\]

\paragraph{Conditional Gaussian formula.}
If
\[
\binom{X}{Y}\sim \mathcal N\!\left(
\binom{\mu_X}{\mu_Y},
\begin{pmatrix}
\Sigma_{XX} & \Sigma_{XY}\\
\Sigma_{YX} & \Sigma_{YY}
...
\right),
\]
then $X\mid Y=y$ is Gaussian with conditional mean and covariance:
\[
\E[X\mid Y=y]=\mu_X+\Sigma_{XY}\Sigma_{YY}^{-1}(y-\mu_Y),
\qquad
\Cov(X\mid Y=y)=\Sigma_{XX}-\Sigma_{XY}\Sigma_{YY}^{-1}\Sigma_{YX}.
\]
Finance preview: Brownian motion is a Gaussian process; these formulas foreshadow the computations used later.
...
\[
\text{(finite variation / drift)}\quad
\Big(\partial_t f + a_t\,\partial_x f + \tfrac12 b_t^2\,\partial_{xx} f\Big)(t,X_t)\dd t,
\qquad
\text{(local martingale)}\quad
\big(b_t\,\partial_x f\big)(t,X_t)\dd W_t.
\]
